...
# Check which NTP server/service is currently active
Get-NtpConf
...
The NTP tools (`Test-Time`, `Get-NtpConf`, `Set-NtpConf`) build the same way and share a single `NtpCheck.dll`, whether you build it on Windows or Linux:
```powershell
cd src\ps-ntpcheck
